(ns sophos.main
  "Kotodama WASM entrypoint for the Sophos clean-room actor (L4) — Clojure port.
  ...
  facts (`sophos.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sophos")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sophos.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sophos-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sophos-compat/ready)
